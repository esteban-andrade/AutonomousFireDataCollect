...
{
  UsbPacketDriver::UsbPacketDriver(int vendor_id, int product_id) : is_ok_(true),
                                                                    states_(INIT),
                                                                    context_(NULL),
  ...
  bool UsbPacketDriver::get_next_packet(std::vector<unsigned char> &packet)
  {
    ...
    if (!packet_deque_.data.empty())
    {
      buffer next_data = packet_deque_.data.front();
      ...
      packet_deque_.data.pop_front();
      packet_deque_.data.shrink_to_fit();
      //packet_queue_.mtx.unlock();
      return true;
    }
    // packet_queue_.mtx.unlock();
    packet_deque_.data.clear();
    packet_deque_.data.shrink_to_fit();
    return false;
  }
  ...
  void UsbPacketDriver::read(const char ep[], char EP_error[], int r, int actual_length, unsigned char buf[])
  {
    ...
    // packet_queue_.mtx.lock();
    packet_deque_.data.push_front(usb_buffer_);
    packet_deque_.data.shrink_to_fit();
    //packet_queue_.mtx.unlock();
  }
  ...
  int UsbPacketDriver::poll_data(void)
  {
    ...
    case INIT:
      ...
      r = libusb_control_transfer(devh_, 1, 0x0b, 0, 2, data, DEFAULT_LENGTH_PRE_SET, TIME_OUT_TIME_CONTROL_TRANSFER);
      if (r < 0)
      {
    ...
    case INIT_1:
      ...
      r = libusb_control_transfer(devh_, 1, 0x0b, 0, 1, data, DEFAULT_LENGTH_PRE_SET, TIME_OUT_TIME_CONTROL_TRANSFER);
      if (r < 0)
      {
        ROS_ERROR("Control Out error %d\n", r);
        error_code_ = r;
    ...
    case INIT_2:
      ...
      r = libusb_control_transfer(devh_, 1, 0x0b, 1, 1, data, DEFAULT_LENGTH_PRE_SET, TIME_OUT_TIME_CONTROL_TRANSFER);
      if (r < 0)
      {
        ROS_ERROR("Control Out error %d\n", r);
        error_code_ = r;
